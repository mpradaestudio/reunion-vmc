...
<div class="d-flex justify-content-between align-items-center mb-4">
    <h1 class="h2 mb-0"><i class="bi bi-gear me-2"></i>Configuración del Sistema</h1>
</div>
...
<div class="row">
    <div class="col-md-8">
        ...
        <div class="card mb-4" id="card-bosquejos">
            <div class="card-header d-flex justify-content-between align-items-center">
                <h5 class="mb-0"><i class="bi bi-journals me-2"></i>Bosquejos</h5>
            ...
            <div class="card-body">
                <p class="text-muted small mb-3">
                    Discursos públicos disponibles. Se usan en Reunión Fin de Semana para seleccionar
                    el tema del Discurso Público. Puedes importarlos masivamente desde un CSV.
                </p>
                ...
                <div class="input-group input-group-sm mb-3">
                    ...
                    <input type="text" class="form-control" id="filtroBosquejos"
                           placeholder="Filtrar por número o título…" autocomplete="off">
                    <button class="btn btn-outline-secondary" type="button"
                            id="limpiarFiltroBosquejos" style="display:none;">
    ...
    <div class="col-md-4">
            ...
            <div class="card-body">
                ...
                <div class="mb-3">
                    ...
                    <div class="progress" style="height:5px;">
                        <div class="progress-bar bg-success"
                             style="width:<?php echo $stats['personas'] > 0
                                 ? round($stats['personas_activas'] / $stats['personas'] * 100)
                                 : 0; ?>%">
                        </div>
                    </div>
                </div>
                ...
                <div>
                    <small class="text-muted">
                        <i class="bi bi-clock-history"></i> Último scraping:<br>
                        <strong>
                            <?php echo $stats['ultimo_scraping'] !== 'Nunca'
                                ? date('d/m/Y H:i', strtotime($stats['ultimo_scraping']))
                                : 'Nunca'; ?>
                        </strong>
                    </small>
                </div>
        ...
        <div class="card mb-4">
            <div class="card-header">
                <h5 class="mb-0"><i class="bi bi-tools me-2"></i>Mantenimiento</h5>
            </div>
            <div class="card-body">
                ...
                <div class="d-grid gap-2">
                    ...
                    <a href="../database.sql" class="btn btn-outline-secondary btn-sm" download>
    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title"><i class="bi bi-clock-history"></i> Historial de Scraping</h5>
            ...
            <div class="modal-body">
                ...
                <div class="table-responsive">
                    <table class="table table-sm table-hover">
                        <thead>
                            <tr>
                                <th>Fecha</th><th>URL</th><th>Programas</th><th>Estado</th>
                            </tr>
                        </thead>
                        <tbody>
                            <?php foreach ($historial as $h): ?>
                            <tr>
                                <td><small><?php echo date('d/m/Y H:i', strtotime($h['fecha_scraping'])); ?></small></td>
                                <td>
                                    <small class="text-truncate d-inline-block" style="max-width:200px;">
                                        <?php echo htmlspecialchars($h['url_procesada']); ?>
                                    </small>
                                </td>
                                <td><span class="badge bg-info"><?php echo $h['num_programas_extraidos']; ?></span></td>
                                <td>
                ...
                <div class="modal-body">
                    ...
                    <div class="form-check form-switch mb-2">
                        <input class="form-check-input bosq-no-presentar-toggle" type="checkbox"
                               id="bosquejoEditNoPresentar" name="no_presentar" role="switch">
                        <label class="form-check-label fw-semibold" for="bosquejoEditNoPresentar">
                            No presentar
                        </label>
                    </div>
                    <div id="notaNoPresentarWrap" style="display:none;">
                        ...
                        <textarea class="form-control" id="bosquejoEditNota"
                                  ...
                                  placeholder="Ej: Discurso ya presentado recientemente…"></textarea>
                        <small class="text-muted">Esta nota aparecerá como aviso en Fin de Semana.</small>
                    </div>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancelar</button>
...
/* ── Bosquejos ──────────────────────────────────────────────── */
const Bosquejos = {
    pagina: 1, porPagina: 20, q: '', total: 0, timer: null,
    ...
    buildRow(b) {
        ...
        return `
            <div class="list-group-item d-flex justify-content-between align-items-center"
                ...
                <span>
                    <span class="badge bg-secondary me-2">${numE}</span>${tituloE}${badge}
                </span>
    ...
    cargar(pagina, porPagina, q) {
        ...
        $.ajax({
            url: '../api/bosquejos.php', method: 'GET', dataType: 'json',
            ...
            success: (res) => {
                ...
                $('#bosqInfo').html(`Mostrando <strong>${desde}–${hasta}</strong> de <strong>${total}</strong> bosquejos`);
                this.buildPaginator(page, totalPages);
            },
            error: () => {
                $('#lista-bosquejos').html('<div class="list-group-item text-danger">Error al cargar bosquejos</div>');
            }
        });
    },
    ...
    init() {
        ...
        $(document).on('input', '#filtroBosquejos', (e) => {
            ...
            this.timer = setTimeout(() => { this.cargar(1, undefined, val); }, 350);
        });
...
$(document).on('submit', '#formNuevoBosquejo', function (e) {
    ...
    apiPost('../api/bosquejos.php', { action: 'create', numero, titulo }, function (res) {
...
$(document).on('hidden.bs.modal', '#modalNuevoBosquejo', function () {
    $('#bosquejoNumero').val(''); $('#bosquejoTitulo').val('');
});

$(document).on('click', '.btn-editar-bosquejo', function () {
    $('#bosquejoEditId').val($(this).data('id'));
    $('#bosquejoEditNumero').val($(this).data('numero'));
    $('#bosquejoEditTitulo').val($(this).data('titulo'));
    const noPres = parseInt($(this).data('no-presentar') || '0');
    $('#bosquejoEditNoPresentar').prop('checked', noPres === 1);
    $('#bosquejoEditNota').val($(this).data('nota') || '');
